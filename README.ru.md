...
Основной сценарий работы строится вокруг **TaskTable**: это недельная таблица списаний с интерактивным редактированием, быстрым добавлением времени, проверкой и нормализацией длительности (`1d`, `2h`, `30m`), группировкой по задачам и итогами по дням. После входа через **Yandex OAuth** пользователь получает свои задачи и может вести учёт времени в одном интерфейсе.

Помимо учёта времени, в дашборде есть режимы **планирования**, **поиска** и **отчётности**. В планировании можно подбирать задачи в спринт, управлять планом работ, анализировать загрузку сотрудников и сравнивать план/факт/остаток. В поиске доступен полнотекстовый просмотр задач и комментариев, а в отчётах можно смотреть агрегированные данные по периодам.

Интерфейс построен на React + Vite, TypeScript и Material UI. Он поддерживает переключение между своими данными и данными команды, постраничную и недельную навигацию, а также уведомления о сохранении, ошибках и проблемах валидации.
