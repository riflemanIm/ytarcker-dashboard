...
The main workflow is built around **TaskTable**: a weekly worklog grid with inline editing, quick time entry, duration validation and normalization (`1d`, `2h`, `30m`), issue grouping, and day-by-day totals. After signing in through **Yandex OAuth**, users can load their issues and manage time in a single interface.

Beyond time tracking, the dashboard includes **planning**, **search**, and **reporting** modes. Planning helps teams select sprint tasks, manage work plans, analyze staff capacity, and compare plan vs actual vs remaining effort. Search provides full-text access to tasks and comments, while reporting aggregates data by period.

The UI is built with React + Vite, TypeScript, and Material UI. It supports switching between personal and team views, week-based navigation, and status feedback for saves, errors, and validation issues.
